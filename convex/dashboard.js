...
export const getUserBalances = query({
  handler: async (ctx) => {
    ...
    let youOwe = 0; // Total amount user owes other
    let youAreOwed = 0; // Total amount other owe the user
    ...
    for (const e of expenses) {
      ...
      if (isPayer) {
        for (const s of e.splits) {
          // Skip's user own split or already paid splits
          if (s.userId === user._id || s.paid) continue;

          // Add to amount owned to user
          youAreOwed += s.amount;
    ...
    for (const s of settlements) {
        if (s.paidByUserId === user._id) {
            ...
            // Someone paid by the user -> reduces what they owe the user
            youAreOwed -= s.amount;
    ...
    const youAreOwedByList = [];   // List of the people who owe the user
...
export const getMonthlySpending = query({
    ...
    // Sort the month by (chronological order)
...
export const getUserGroups = query({
    handler: async (ctx) => {
        ...
        // Filter to only include groups when the user is a member
        ...
        const enhancedGroups = await Promise.all(
            ...
            settlements.forEach((settlement) => {
                if (settlement.paidByUserId === user._id) {
                    // User paid someone in the group - increases the user's balance
                    balance += settlement.amount;
                } else {
                    // Someone paid the user - decreases the user's balance
                    balance -= settlement.amount;
                }
            });
