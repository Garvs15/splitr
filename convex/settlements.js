    ...
    handler: async (ctx, args) => {
        ...
        if (args.entityType === "user") {
            ...
            const otherUserExpenses = await ctx.db.query("expenses")
                .withIndex("by_user_and_group", (q) => q.eq("paidByUserId", me._id).eq("groupId", undefined)
                ).collect();
            ...
            for (const exp of expenses) {
                ...
                const involvesThem = exp.paidByUserId === other._id ||
            ...
            const mySettlements = await ctx.db.query("settlements").withIndex("by_user_and_group", (q) => q.eq("paidByUserId", me._id).eq("groupId", undefined)
            ).collect();

            const otherUserSettlements = await ctx.db.query("settlements").withIndex("by_user_and_group", (q) => q.eq("paidByUserId", (q) => other._id).eq("groupId", undefined)).collect();
            ...
            return {
                ...
                youAreOwed: owed,
                youOwe: owing,
                netBalance: owed - owing,  // + => you should receive, - => you should play
            };
            ...
            group.members.forEach((m) => {
                if (m.userId !== m._id) balances[m.userId] = { owed: 0, owing: 0 };
            });

            // -------- apply expenses
            for (const exp of expenses) {
                if (exp.paidByUserId === me._id) {
                    // I paid; others may owe them
