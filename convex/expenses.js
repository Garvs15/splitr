...
export const getExpensesBetweenUsers = query({
    ...
    handler: async (ctx, { userId }) => {
        ...
        const myPaid = await ctx.db.query("expenses")
        ...
        const expenses = candidateExpenses.filter(e => {
            ...
            const themInInvolved = e.paidByUserId === userId || themInSplits;

            return meInvolved && themInInvolved;
        });

        expenses.sort((a, b) => b.date - a.date);

        /* 3. Settlements between the 2 of us (groupId = undefined)  */
        ...
        for (const s of settlements) {
            if (s.paidByUserId === me._id) {
                balance += s.amount;  // paid them back
            } else {
