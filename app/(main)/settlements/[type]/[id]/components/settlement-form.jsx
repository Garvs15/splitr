...
export default function SettlementForm({ entityType, entityData, onSuccess }) {
    ...
    const handleUserSettlement = async (data) => {
        ...
        try {
            ...
            const paidByUserId = data.paymentType === "youPaid"
                ? currentUser._id : entityData.counterpart.userId;

            const receivedByUserId = data.paymentType === "youPaid"
            ...
            await createSettlement.mutate({
                ...
                paidByUserId,
                receivedByUserId
                // No groupId for user settlement
            });
    ...
    const handleGroupSettlement = async (data, selectedUserId) => {
        ...
        try {
            ...
            if (!selectedUser) {
                toast.error("Selected user not found in the group!");
                return;
            }
    ...
    // For group settlements, the user need to select a member
    ...
    if (entityType === "user") {
        ...
        const netBalance = entityType.netBalance;
        ...
        return (
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="space-y-2">
                    ...
                    <RadioGroup
                        defaultValues="youPaid"
                        {...register("paymentType")}
                        className="flex flex-col space-y-2"
                        onValueChange={(value) => {
                            // This manual approach is needed because readiogroup doesn't work directly with react-hook-form 
    ...
    // Render for for group settlement
    if (entityType === "group") {
                    ...
                    <div className="space-y-2">
                        {groupMembers.map((member) => {
                            ...
                            return (
                                <div
                                    key={member.userId}
                                    className={`border rounded-md p-2 cursor-pointer transition-colors ${isSelected ? "border-primary bg-primary/5"
                                            : "hover:bg-muted/50"}`}
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className={`
                                        font-medium ${isOwing
                                                ? "text-green-600" : "isOwed"
                                                    ? "text-red-600" : ""
                                            }`}>
                                            {isOwing ? `They owe you $${Math.abs(member.netBalance).toFixed(2)}`
                                                : isOwed ? `You owe $${Math.abs(member.netBalance).toFixed(2)}`
                                                    : "Settled up!"
                                            }
                                        </div>
                                    </div>
                    ...
                    <>
                        ...
                        <div className="space-y-2">
                            ...
                            >
                                ...
                                <div className="flex items-center space-x-2 border rounded-md p-3">
                                    <RadioGroupItem id="theyPaid" value="theyPaid" />
                                    <Label htmlFor="theyPaid" className="flex-grow cursor-pointer">
                                        <div className="flex items-center">
                                            <Avatar className="h-6 w-6 mr-2">
                                                <AvatarImage src={groupMembers.find(
                                                    (m) => m.userId === selectedGroupMemberId
                                                )?.imageUrl
                                                }
                                                />
